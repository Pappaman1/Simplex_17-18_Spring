...
Simplex::MyOctant::MyOctant(uint a_nMaxLevel, uint a_nIdealEntityCount)
{
	...
	for (int i = 0; i < m_pEntityMngr->GetEntityCount(); i++)
	{
		...
		std::cout << m_v3Min.x << std::endl;
		std::cout << m_v3Min.y << std::endl;
		std::cout << m_v3Min.z << std::endl;
		std::cout << m_v3Max.x << std::endl;
		std::cout << m_v3Max.y << std::endl;
		std::cout << m_v3Max.z << std::endl;
	}
...
void Simplex::MyOctant::Display(vector3 a_v3Color)
{
	...
	int nIndex = m_pMeshMngr->GenerateCube(m_fSize, a_v3Color);
	//m_pMeshMngr->AddCubeToRenderList(m_fSize,1);
	m_pMeshMngr->AddMeshToRenderList(nIndex, tempMat4, 2);
	if (m_uChildren != 0)
	{
		for (auto& i:m_pChild)
		{
